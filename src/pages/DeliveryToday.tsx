...
const shipmentStatusMeta: Record<string, { label: string; icon: typeof Truck; colorClass: string; bgClass: string }> = {
  confirmed: { label: "Confirmed", icon: Package, colorClass: "text-[var(--color-info)]", bgClass: "bg-[var(--color-alert-info-bg)] border-[var(--color-info)]" },
  ...
  delivered: { label: "Delivered", icon: CheckCircle, colorClass: "text-[var(--color-success)]", bgClass: "bg-[var(--color-alert-success-bg)] border-[var(--color-success)]" },
};
...
function ShipmentCard({
  ...
  const meta = shipmentStatusMeta[shipment.status] ?? shipmentStatusMeta.confirmed;
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-[var(--color-text-secondary)]">· Shipment {shipment.shipment_index}</span>
          </div>
      ...
      {items.length > 0 && (
        <div className="space-y-1.5 mb-3">
          ...
          {items.length > 3 && (
            <p className="text-xs text-[var(--color-text-helper)]">+{items.length - 3} more item(s)</p>
          )}
        </div>
      )}
      ...
      {shipment.delivered_at && shipment.delivered_signed_by && (
        <p className="text-xs text-[var(--color-success)]">
          ✓ Signed by {shipment.delivered_signed_by}
        </p>
      )}
    </div>
  );
...
export default function DeliveryToday() {
  ...
  // Find the best demo date — today if there are shipments, otherwise the nearest date with shipments
  const demoDate = useMemo(() => {
    ...
    // Find nearest future or most recent date
  ...
  // Filter shipments for active date
  ...
  const kpis = useMemo(() => {
    ...
    const pending = todayShipments.filter((s) => s.status === "confirmed" || s.status === "being_prepared").length;
    const totalItems = lineItems?.filter((li) => todayShipments.some((s) => s.id === li.shipment_id)).reduce((sum, li) => sum + li.quantity, 0) ?? 0;
    return { total, delivered, inTransit, pending, totalItems };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--color-text-secondary)]">
            Shipments expected on {format(activeDate, "EEEE, dd MMMM yyyy")}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-2 px-3 h-9 rounded-[var(--border-radius-sm)] border border-[var(--color-border-subtle)] text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-bg-layer-01)]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard label="Total shipments" value={kpis.total} icon={Package} color="bg-[var(--color-alert-info-bg)] text-[var(--color-info)]" />
        <KpiCard label="Delivered" value={kpis.delivered} icon={CheckCircle} color="bg-[var(--color-alert-success-bg)] text-[var(--color-success)]" />
        <KpiCard label="In transit" value={kpis.inTransit} icon={Truck} color="bg-[var(--color-alert-warning-bg)] text-[var(--color-warning)]" />
        <KpiCard label="Total items" value={kpis.totalItems} icon={Clock} color="bg-[var(--color-bg-layer-01)] text-[var(--color-text-secondary)]" />
      </div>
      ...
      {kpis.total > 0 && (
        <div className="rounded-[var(--border-radius-sm)] border border-[var(--color-border-subtle)] bg-[var(--color-bg-layer-02)] p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-[var(--color-text-primary)]">Reception progress</span>
            <span className="text-sm text-[var(--color-text-secondary)]">{kpis.delivered}/{kpis.total} received</span>
          </div>
      ...
      {todayShipments.length === 0 ? (
        <div className="rounded-[var(--border-radius-sm)] border border-[var(--color-border-subtle)] bg-[var(--color-bg-layer-02)] p-12 text-center">
          ...
          <p className="text-[var(--color-text-secondary)] font-medium">No deliveries expected on this date</p>
          <p className="text-xs text-[var(--color-text-helper)] mt-1">Use the date navigator to browse other dates</p>
        </div>
      ) : (
